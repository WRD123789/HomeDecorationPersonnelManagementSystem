...
ProjectManagerTable::ProjectManagerTable(QWidget *parent)
    : InfoTable(parent)
{
    init();
...
QVector<QString> ProjectManagerTable::setColName()
{
...
void ProjectManagerTable::loadData()
{
    empMap.clear();
    auto empVec = empModel.queryAllByIdentity(QString("项目经理"));
    for (auto& emp : empVec) {
        empMap.insert(emp.getId(), emp);
    }
}

void ProjectManagerTable::showData()
{
    int row = 0;
    ...
    for (auto& [empId, emp] : empMap.toStdMap()) {
        ...
        connect(button1, &QPushButton::clicked, this, &ProjectManagerTable::onEmpInfoBtnClicked);
...
void ProjectManagerTable::onEmpInfoBtnClicked()
{
